...
#ifdef _EXHALOS
void place_halos_Mglobal(long NHalosTot, float *HaloMass, long Nlin, long NTotPart, float *PartX, float *PartY, float *PartZ, float L, float mp, float Nmin, float *HaloX, float *HaloY, float *HaloZ){
...
//----------------------------------- Particles assigned to grid
	...
	for (ihalo=0;ihalo<NHalosTot;ihalo++){
		...
		do {
			...
			R=R200_from_mass(HaloMass[ihalo]);
			HaloR[ihalo]=R;
			check = check_HaloR(ihalo,HaloX,HaloY,HaloZ,HaloR);
			if (check==0){
				#ifdef _DEBUG
				fprintf(stderr,"Refused part : %ld. Excluded = %ld\n",ipart,excluded[ipart]);
				#endif
...
//Initiallising -------------------------------------------------
	...
	mpart=mp;
	if (rank_cell!=0.)
		fprintf(stderr,"Using RANKED approach, with lcell approx %d\n",rank_cell);
	else {	
		rank_cell = 2*R200_from_mass(HaloMass[0]);
		fprintf(stderr,"Using RANKED approach, with lcell computed automatically. M[0]=%e, R[0]=%f, lcell aprox %f\n",HaloMass[0],R200_from_mass(HaloMass[0]),rank_cell);
	}

	NCells = (long) (L/rank_cell);	
	lcell = L/NCells;
...
#endif
#ifdef _EXHALOS
	fprintf(stderr,"#def _EXHALOS <- Not compatible!\n");
	exit(0);
#endif
...
long select_cell(long *x, long *y, long *z, float *MassArray, int N, float M) {
	...
	fprintf(stderr,"ERROR: no cell selected: TotMass = %e, rnd = %e. th= %e, tol =%e\n",TotMass,d_rand,threshold,tol);
	return -1;		
}
