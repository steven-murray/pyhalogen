...
long select_cell_three(long *, long *, long *); 
...
double **CumulativePartialMass, *CumulativeTotMass; 
...
//Initiallising -------------------------------------------------
	long i,j,k,lin_ijk,check,alpha;
	...
	CumulativeTotMass = (double *) calloc(NCells, sizeof(double));
  	if(CumulativeTotMass == NULL) {
    		fprintf(stderr,"select_cell_three(): could not allocate %ld * %ld array for CumulativeTotMass[]\nABORTING",NCells);
    		exit(-1);
	}
	CumulativePartialMass = (double **) calloc(NCells, sizeof(double *));
  	if(CumulativePartialMass == NULL) {
    		fprintf(stderr,"select_cell_three(): could not allocate %ld array for CumulativePartialMass[]\nABORTING",NCells);
    		exit(-1);
	}	
	for (i=0;i<NCells;i++){
		CumulativePartialMass[i] = (double *) calloc(NCells*NCells, sizeof(double));
        	if(CumulativePartialMass[i] == NULL) {
                	fprintf(stderr,"select_cell_three(): could not allocate  %ld array for CumulativeMass[%ld]\nABORTING",NCells,i);
        	        exit(-1);
	        }
	}
...
#endif
#ifdef AK
	fprintf(stderr,"#def AK\n");
#endif
#ifdef _OMP
	fprintf(stderr,"#def _OMP\n");
#endif
#ifdef _RNDCELL
...
//----------------------------------- Particles assigned to grid
	...
	for (ihalo=0;ihalo<NHalosTot;ihalo++){
		...
		switch(alpha){
			...
			case 2:
				...
				break;
			case 3:
				lin_ijk = select_cell_three(&i,&j,&k);
				break;
			default:
		...
		#endif
		if ((MassLeft)[lin_ijk]==0.0){
			fprintf(stderr,"Wrong association: Cell selected had M=0.\n");
			exit(0);
		}
	...
	free(CumulativePartialMass);
	free(CumulativeTotMass);
}
...
//Initiallising -------------------------------------------------
	...
	fprintf(stderr,"BOX = %f  lcell =%f   rho_ref = %e  invL %f\n",L,lcell,rho_ref,invL);
	fprintf(stderr,"mp = %f  fract =%f  mp used =%e\n",mp,fract,mpart);
...
long select_cell(long *x,long *y, long *z) {
	long i,j,k,lin_ijk;
	...
	for (k=0;k<NCells;k++){
		...
		TotMass += (double) MassLeft[lin_ijk];
		 #else
		TotMass += (double) (MassLeft[lin_ijk]*MassLeft[lin_ijk]*1.0e-30*MassLeft[lin_ijk]);
		 #endif
		#else
...
long select_cell_two(long *x,long *y, long *z) {
	long i,j,k,lin_ijk,jk,i_low,i_up,i_mid,jk_up,jk_mid,jk_low;
	double TotMass = 0., d_rand;
	
	
	#pragma omp parallel private(i,TotMass,jk,lin_ijk)
	for (i=0;i<NCells;i++){
		TotMass=0.0;
		for (jk=0;jk<NCells*NCells;jk++){
			lin_ijk = jk+i*NCells*NCells;
			TotMass += (double) (MassLeft[lin_ijk]*MassLeft[lin_ijk]);
			CumulativePartialMass[i][jk]=TotMass;
		}
		CumulativeTotMass[i]=TotMass;
	}
	
	TotMass=0.0;
	for (i=1;i<NCells;i++){
		CumulativeTotMass[i]+=CumulativeTotMass[i-1];
	}
	TotMass=CumulativeTotMass[NCells-1];
	d_rand = TotMass * ((double)rand()/(RAND_MAX+1.0));
	...
	//binarysearch
        ...
        while(i_low < i_up) {
                i_mid = (i_low+i_up)/2;
                if(d_rand < CumulativeTotMass[i_mid])
                        i_up  = i_mid;
                else
                        i_low = i_mid + 1;
       	}
	if (i_low!=0)
		d_rand -= CumulativeTotMass[i_low-1];	
	jk_low = 0;
	jk_up = NCells*NCells;
	while(jk_low < jk_up){
		jk_mid = (jk_low+jk_up)/2;
                if (d_rand < CumulativePartialMass[i_low][jk_mid])
                        jk_up  = jk_mid;
                else
                        jk_low = jk_mid + 1;
	}
	...
	*y=(jk_low-(*z))/NCells;
	*x=i_low;
	return jk_low+i_low*NCells*NCells;
}
...
#ifdef AK
long select_cell_three(long *x,long *y, long *z) {
	long i,j,k,lin_ijk;
	double TotMass = 0., d_rand;
  ...
  CumulativeMass = (double *) calloc(NCells*NCells*NCells, sizeof(double));
  if(CumulativeMass == NULL) {
    fprintf(stderr,"select_cell_three(): could not allocate %ld * %ld * %ld array for CumulativeMass[]\nABORTING",NCells,NCells,NCells);
    exit(-1);
  }
